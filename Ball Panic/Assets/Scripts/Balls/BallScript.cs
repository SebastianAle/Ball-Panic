...
public class BallScript : MonoBehaviour 
{
	...
	void InstatiateBalls()
	{
		if (this.gameObject.tag != "SmallestBall") 
		{
			ball1 = Instantiate (newBalls);
			ball2 = Instantiate (newBalls);
	...
	void InitializeBallsAndTurnOffCurrentBall()
	{
		...
		{
			if (transform.position.y > 1f && transform.position.y <= 1.3f) 
			{
	...
	{
		if (target.tag == "First Arrow" || target.tag == "Second Arrow" 
			|| target.tag == "First Sticky Arrow" || target.tag == "Second Sticky Arrow") 
		{
			if (gameObject.tag != "SmallestBall") 
			{
